...
int leastOpsExpressTarget(int x, int target) {
    ...
    // At this time, you can get target either by add target times x/x or subtract (x - target)
    // times x/x to x For example, x = 3, target = 2. Then, 3/3 + 3/3 or 3 - 3/3 is possible result
    if (x > target) {
