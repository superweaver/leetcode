...
class Solution {
  public:
    int minDeletionSize(vector<string> &A) {
        int m = A.size();
        int n = A[0].size();
        set<int> columns; // which should be kept
        for (int i = 0; i < n; ++i) {
            columns.insert(i);
        }
  ...
  private:
    void helper(vector<string> &A, set<int> &columns, int row_begin, int row_end, int column) {
        ...
        for (int r = row_begin; r < row_end;) {
            ...
            helper(A, columns, r, next_r, column + 1); // r to next_r and column + 1 to end
            if (next_r < row_end) {
                if (A[r][column] > A[next_r][column]) {
                    columns.erase(column); // have to erase it
                    helper(A, columns, row_begin, row_end, column + 1);
                    // continue to process other column;
                    return;
                }
            }
            r = next_r;
